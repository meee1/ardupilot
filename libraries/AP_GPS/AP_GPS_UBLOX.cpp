...
AP_GPS_UBLOX::_parse_gps(void)
{
    ...
    case MSG_PVT:
        ...
        switch (_buffer.pvt.fix_type) 
        {
            ...
            case 3:
                state.status = AP_GPS::GPS_OK_FIX_3D;
                if (_buffer.pvt.flags & 0b0000010)  // diffsoln
        ...
        state.ground_speed     = _buffer.pvt.gspeed*0.01f;          // m/s
        state.ground_course_cd = wrap_360_cd(_buffer.pvt.head_mot / 1000);       // Heading 2D deg * 100000 rescaled to deg * 100
        state.have_vertical_velocity = true;
        state.velocity.x = _buffer.pvt.velN * 0.01f;
        state.velocity.y = _buffer.pvt.velE * 0.01f;
        state.velocity.z = _buffer.pvt.velD * 0.01f;
        state.have_speed_accuracy = true;
        state.speed_accuracy = _buffer.pvt.s_acc*0.01f;
        _new_speed = true;
        // dop
        if(noReceivedHdop) {
