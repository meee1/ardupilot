...
static THD_FUNCTION(usb_msd_worker, arg) {
  ...
  chRegSetThreadName("usb_msd_worker");
   ...
   osalDbgCheck((msdp != NULL) && msdp->usbp != NULL);
  ...
  while(! chThdShouldTerminateX()) {
    ...
    if (MSG_RESET == status) {
      ...
      osalDbgCheck((&msdp->scsi_target != NULL) && &msdp->cbw != NULL);
...
stm32_watchdog_pat();
     printf("scsiStart done");
    chThdSleepMilliseconds(100);

  msdp->state = USB_MSD_READY;
  msdp->worker = chThdCreateStatic(msdp->waMSDWorker, sizeof(msdp->waMSDWorker), MSD_THD_PRIO, usb_msd_worker, msdp);
}
